...
from models.AU_Aware_Model import generate_model
...
def main():
    ...
    # Data loading code
    train_data = train_data_loader(image_size=112,
                                   annotation_file=project_path+'annotation/set_'+str(args.data_set)+'_train.txt',
                                   bbox_path=project_path+'local_bbox/')
    test_data = test_data_loader(image_size=112,
                                 annotation_file=project_path+'annotation/set_'+str(args.data_set)+'_test.txt',
                                 bbox_path=project_path+'local_bbox/')
...
def train(train_loader, model, criterion, optimizer, epoch, args):
    ...
    for i, (data_dict, target) in enumerate(train_loader):

        images = data_dict['images'].cuda()
...
def validate(val_loader, model, criterion, args):
    ...
    with torch.no_grad():
        for i, (data_dict, target) in enumerate(val_loader):
            images = data_dict['images'].cuda()
            landmarks = data_dict['landmarks']
            target = target.cuda()
